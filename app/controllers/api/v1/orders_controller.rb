...
module Api
  module V1
    class OrdersController < ApplicationController
      def index
        orders = current_user.orders.joins(:items).preload(items: :product).distinct.order(created_at: :desc)
      ...
      def field_opts(attrs = {})
        {
          fields: {
            ...
            order: %i[id status total_price total_items created_at updated_at items payment_id],
            order_item: %i[id product_id quantity price]
          },
          include: %i[items.product],
          params: { total_quantity: current_user.orders.total_quantity, payment_id: current_user.payments.pluck(:order_id, :id).to_h }
        }.merge(attrs)
      end
    end
  end
